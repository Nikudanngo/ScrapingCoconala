...
(async () => { 
  ...
  const promise=()=>{
    ...
    while (n<=1) {  
      ...
      const url = await page.$$eval(urls, elements => { //評価数
        return elements.map(element => element.textContent.href) //検索結果の評価数を取得
      });    
      ...
      const split_rating = rating[index].split(/\r\n|\n/);  //評価数を分割
          if(rating[index] == '-'){  //評価数がundefindedの場合
            allData.push({title:title[index],url:url[index],price:price[index].replace('円',''),rating:'0'});  //配列に格納&評価数を0にする&不要な文字を削除
          }else{
            ...
            if(split_rating[1] == undefined){
              allData.push({title:title[index],url:url[index],price:price[index].replace('円',''),rating:'0'});  //配列に格納&不要な文字を削除
            }else{
            allData.push({title:title[index],url:url[index],price:price[index].replace('円',''),rating:split_rating[1].replace('(','').replace(')','').trim()});  //配列に格納&不要な文字を削除          }
